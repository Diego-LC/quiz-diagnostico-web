...
- permite responder con teclado (`1`–`4` o `A`/`B`/`C`), marcar confianza (`Q`/`G`, `W`/`D`, `E`/`S`) y pausar/reanudar (`P`), avanzando automáticamente;
...
npm run data:generate:v2    # banco v2 usado por la aplicación
```

El generador original comprueba 13 áreas, 15 preguntas por área, tres alternativas, claves asociadas e identificadores únicos. El generador v2 conserva esas preguntas, añade metadatos y completa 24 ítems por área (con alternativas de tres o cuatro opciones y crédito parcial donde corresponde). Las preguntas generadas quedan marcadas como `pilot` y deben revisarse antes de usarse como instrumento formal.
